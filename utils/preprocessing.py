...
from rl_baseline.obs_wrappers import (
    COLORS,
...
class GPT5BaselineTransform:
    ...
    def __call__(self, minibatch):
        ...
        norm_bl_stats = (
            bl_stats * VectorFeaturesWrapper.BLSTAT_NORMALIZATION_STATS
        ).astype(np.float32)
        np.clip(
            norm_bl_stats,
        ...
        cropped_views = np.zeros(
            (np.prod(seq_shape), *self.chw_image_shape), dtype=np.uint8
        )
        for i, (char, color, offset_h, offset_w) in enumerate(
            zip(
                chars.reshape(-1, *tty_shape),
                colors.reshape(-1, *tty_shape),
                offset_hs,
                offset_ws,
            )
        ):
