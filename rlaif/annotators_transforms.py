...
class MessageTransform:
    def __call__(self, pair: Dict[str, np.ndarray]) -> Dict[str, np.ndarray]:
        ...
        dict_to_return["message"] = np.array(
            [
                ["".join([chr(c) for c in row if c != 0]) for row in message]
...
class BlstatsTransform:
    ...
    def blstats_to_str(self, blstats: np.ndarray):
        ...
        bls = " ".join(
            [
                self.blstats_to_index[key][1].format(
                    blstats[self.blstats_to_index[key][0]]
                    if key != "NLE_BL_HUNGER"
                    else self.hunger_num_to_str[
                        int(blstats[self.blstats_to_index[key][0]])
                    ]
                )
    ...
    def __call__(self, pair: Dict[str, np.ndarray]) -> Dict[str, np.ndarray]:
        ...
        dict_to_return["blstats"] = [
            [self.blstats_to_str(bls) for bls in seq] for seq in pair["blstats"]
        ]
        return dict_to_return
