...
class DmlabLevelGenerator(DummySampler):
    ...
    def sample(self, proc_idx):
        ...
        assert (
            self.cfg.num_envs_per_worker == 1
        ), "use populate_cache with 1 env per worker"
        ...
        with timing.timeit("env_init"):
            ...
            env_config = AttrDict(
                worker_index=proc_idx, vector_index=0, env_id=global_env_id
            )
            ...
            if hasattr(env.unwrapped, "level_name"):
                ...
                approx_num_episodes_per_1b_frames = (
                    DMLAB30_APPROX_NUM_EPISODES_PER_BILLION_FRAMES[env_key]
                )
                ...
                env_num_levels_generated = (
                    len(
                        dmlab_level_cache.DMLAB_GLOBAL_LEVEL_CACHE[0].all_seeds[
                            env_level
                        ]
                    )
                    // num_workers_for_env
