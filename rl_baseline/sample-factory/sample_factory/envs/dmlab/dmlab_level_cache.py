...
class DmlabLevelCacheGlobal:
    ...
    def __init__(
        self, cache_dir, experiment_dir, all_levels_for_experiment, policy_idx
    ):
        ...
        lvl_seed_files = Path(os.path.join(cache_dir, "_contributed")).rglob(
        ...
        for lvl_seed_file in lvl_seed_files:
            ...
            self.all_seeds[level] = list(
                set(self.all_seeds[level])
            )  # leave only unique seeds
            log.debug(
                "Level %s has %d total seeds available",
                level,
        ...
        for used_seeds_file in used_seeds_files:
            ...
            log.debug(
                "%d seeds already used in this experiment for level %s",
        ...
        for lvl in all_levels_for_experiment:
            ...
            log.debug(
                "Env %s has %d remaining unused seeds",
                lvl,
    ...
    def record_used_seed(self, level, seed):
        ...
        log.debug(
            "Updated number of used seeds for level %s (%d)",
            level,
...
def dmlab_ensure_global_cache_initialized(
    experiment_dir, all_levels_for_experiment, num_policies, level_cache_dir
):
    ...
    for policy_id in range(num_policies):
        ...
        cache = DmlabLevelCacheGlobal(
            level_cache_dir, experiment_dir, all_levels_for_experiment, policy_id
        )
