...
from sample_factory.algorithms.utils.algo_utils import (
...
from sample_factory.envs.dmlab.dmlab_level_cache import (
    dmlab_ensure_global_cache_initialized,
)
from sample_factory.envs.dmlab.dmlab_model import dmlab_register_models
from sample_factory.envs.dmlab.wrappers.reward_shaping import (
...
DMLAB_ENVS = [
    DmLabSpec(
        "dmlab_benchmark", "contributed/dmlab30/rooms_collect_good_objects_train"
    ),
    ...
    DmLabSpec(
        "dmlab_benchmark_slow_reset", "contributed/dmlab30/rooms_keys_doors_puzzle"
    ),
    ...
    DmLabSpec(
        "dmlab_sparse_doors", "contributed/dmlab30/explore_obstructed_goals_large"
    ),
...
def dmlab_extra_summaries(policy_id, policy_avg_stats, env_steps, summary_writer, cfg):
    ...
    for level_idx, level in enumerate(all_levels):
        ...
        summary_writer.add_scalar(
            f"_dmlab/{level_key}_human_norm_score", human_normalized_score, env_steps
        )
        summary_writer.add_scalar(
            f"_dmlab/capped_{level_key}_human_norm_score",
            capped_human_normalized_score,
            env_steps,
        )

    assert (
        len(level_mean_scores_normalized)
        == len(level_mean_scores_normalized_capped)
        == len(all_levels)
    )
    ...
    summary_writer.add_scalar(
        f"_dmlab/000_mean_human_norm_score", mean_normalized_score, env_steps
    )
    summary_writer.add_scalar(
        f"_dmlab/000_capped_mean_human_norm_score",
    ...
    if target_objective_stat not in policy_avg_stats:
        policy_avg_stats[target_objective_stat] = [
            deque(maxlen=1) for _ in range(cfg.num_policies)
        ]

    policy_avg_stats[target_objective_stat][policy_id].append(
        capped_mean_normalized_score
    )
...
def ensure_initialized(cfg, env_name):
    ...
    dmlab_ensure_global_cache_initialized(
        experiment_dir(cfg=cfg), all_levels, num_policies, level_cache_dir
    )
