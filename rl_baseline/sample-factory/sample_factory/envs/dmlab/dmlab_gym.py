...
class DmlabGymEnv(gym.Env):
    def __init__(
        ...
        self.env_uses_level_cache = (
            False  # will be set to True when this env instance queries the cache
        )
        ...
        self.action_list = np.array(
            self.action_set, dtype=np.intc
        )  # DMLAB requires intc type for actions
        ...
        if self.with_instructions:
            self.observation_space.spaces[self.instructions_observation] = (
                gym.spaces.Box(
                    low=0,
    ...
    def format_obs_dict(self, env_obs_dict):
        ...
        if instr is not None:
            ...
            for i, word in enumerate(instr_words):
                self.instructions[i] = string_to_hash_bucket(
                    word, DMLAB_VOCABULARY_SIZE
                )
    ...
    def reset(self):
        if self.use_level_cache:
            ...
            self.last_reset_seed = self.curr_cache.get_unused_seed(
    ...
    def write(self, key, pk3_path):
        """Environment object itself acts as a proxy to the global level cache."""
        log.debug(
            "Add new level to cache! Level %s seed %r key %s",
            self.level_name,
