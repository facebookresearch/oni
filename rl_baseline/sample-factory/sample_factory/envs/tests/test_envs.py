...
def test_env_performance(make_env, env_type, verbose=False):
    ...
    log.debug(
        "Took %.3f sec to collect %d frames on one CPU, %.1f FPS",
        t.experience,
        total_num_frames,
        fps,
    )
...
@unittest.skipUnless(
    vizdoom_available(), "Please install VizDoom to run a full test suite"
)
class TestDoom(TestCase):
    ...
    def make_env_singleplayer(env_config):
        ...
        return make_doom_env(
            "doom_benchmark", cfg=default_doom_cfg(), env_config=env_config
        )
    ...
    def test_doom_two_color(self):
        ...
        test_env_performance(
            lambda env_config: make_doom_env(
                "doom_two_colors_easy", cfg=default_doom_cfg()
            ),
            "doom",
            verbose=False,
        )
...
class TestDmlab(TestCase):
    ...
    def make_env(env_config):
        ...
        return make_dmlab_env(
            "dmlab_nonmatch", cfg=default_cfg(env="dmlab_nonmatch"), env_config=None
        )
