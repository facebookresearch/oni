...
def retry_dm(
    exception_class=Exception, num_attempts=3, sleep_time=1, should_reset=False
):
    def decorator(func):
        @wraps(func)
...
def udp_port_num(env_config):
    ...
    port_to_use = (
        DEFAULT_UDP_PORT + 100 * env_config.worker_index + env_config.vector_index
    )
    return port_to_use
...
def init_multiplayer_env(make_env_func, player_id, env_config, init_info=None):
    ...
    env.seed(
        env.unwrapped.worker_index * 1000 + env.unwrapped.vector_index * 10 + player_id
    )
    return env
...
class MultiAgentEnvWorker:
    ...
    def _init(self, init_info):
        log.info(
            "Initializing env for player %d, init_info: %r...",
            self.player_id,
            init_info,
        )
        env = init_multiplayer_env(
            self.make_env_func, self.player_id, self.env_config, init_info
        )
...
class MultiAgentEnv(gym.Env, RewardShapingInterface):
    ...
    def _ensure_initialized(self):
        ...
        self.workers = [
            MultiAgentEnvWorker(
                i, self.make_env_func, self.env_config, reset_on_init=self.reset_on_init
            )
        ...
        log.debug(
            "%d agent workers initialized for env %d!",
            len(self.workers),
    ...
    @retry_dm(
        exception_class=Exception, num_attempts=3, sleep_time=1, should_reset=True
    )
