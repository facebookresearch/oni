...
from sample_factory.envs.doom.action_space import (
    doom_action_space,
...
from sample_factory.envs.doom.wrappers.observation_space import (
    SetResolutionWrapper,
    resolutions,
)
from sample_factory.envs.doom.wrappers.reward_shaping import (
...
from sample_factory.envs.doom.wrappers.scenario_wrappers.gathering_reward_shaping import (
...
DEATHMATCH_REWARD_SHAPING = (
    DoomRewardShapingWrapper,
    dict(
        reward_shaping_scheme=REWARD_SHAPING_DEATHMATCH_V1,
...
DOOM_ENVS = [
    ...
    DoomSpec(
        "doom_defend_the_center_flat_actions",
    ...
    DoomSpec(
        "doom_defend_the_line", "defend_the_line.cfg", doom_turn_and_attack_only(), 1.0
    ),
    DoomSpec(
...
def make_doom_env_impl(
    ...
    **kwargs,
    ...
    if player_id is None:
        ...
        from sample_factory.envs.doom.multiplayer.doom_multiagent import (
            VizdoomEnvMultiplayer,
        )
    ...
    if env_config is None:
        ...
        and env_config.vector_index == 0
        and (player_id is None or player_id == 0)
    ):
        should_record = True
    ...
    if num_bots > 0:
        bot_difficulty = (
            cfg.start_bot_difficulty if "start_bot_difficulty" in cfg else None
        )
    ...
    log.info(
        "Doom resolution: %s, resize resolution: %r", resolution, (cfg.res_w, cfg.res_h)
    )
...
def make_doom_multiplayer_env(doom_spec, cfg=None, env_config=None, **kwargs):
    ...
    def make_env_func(player_id):
        return make_doom_env_impl(
            ...
            num_bots=num_bots,
            skip_frames=(
                1 if is_multiagent else skip_frames
            ),  # multi-agent skipped frames are handled by the wrapper
    ...
    if is_multiagent:
        ...
        from sample_factory.envs.doom.multiplayer.doom_multiagent_wrapper import (
            MultiAgentEnv,
        )
