...
def tb_version():
    tb_version_proc = subprocess.Popen(
        ["tensorboard", "--version"], stdout=subprocess.PIPE
    )
...
def main():
    parser = argparse.ArgumentParser(
        description=r"Launch tensorboard on multiple directories in an easy way."
    )
    parser.add_argument(
        "--dir", default="./train_dir", help="Base folder with summaries"
    )
    parser.add_argument(
        "--port", default=6006, type=int, help="The port to use for tensorboard"
    )
    ...
    parser.add_argument(
        "--reload_interval", type=int, default=60, help="How often to reload data"
    )
    parser.add_argument(
        "filters", nargs="+", type=str, help="directories in train_dir to monitor"
    )
