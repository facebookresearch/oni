...
def arg_parser(argv=None, evaluation=False):
    ...
    parser.add_argument(
        ...
        default=0.0,
        type=float,
        help="The coefficient for the counts bonus on messages only.",
    )
    parser.add_argument(
        "--beta_count_exponent",
        default=3,
        type=float,
        help="The exponent for the counts.",
    )
    # this parameter is for the Motif and online ranking code
    # TODO: rename? create another hyper-param for online motif, keep this for Motif
    parser.add_argument(
        "--eps_threshold_quantile",
        default=-0.5,
        type=float,
        help="The threshold over which the reward is activated",
    )
    parser.add_argument(
        "--rew_norm",
        default=True,
        type=str2bool,
        help="To normalize or not the intrinsic reward.",
    )
    parser.add_argument(
        "--root_env",
        type=str,
        default="NetHackScore-v1",
        required=False,
        help="Fully-qualified environment name in the form envfamily_envname, e.g. atari_breakout or doom_battle",
    )
    parser.add_argument(
        ...
        default=0,
        type=int,
        help="The checkpoint number to load the reward model from.",
    )
    parser.add_argument(
        "--wandb", type=str2bool, default=False, help="To log or not on wandb"
    )

    # Intrinsic reward arg
    parser.add_argument(
        "--reward_encoder",
        default="nle_torchbeast_encoder",
        type=str,
        help="encoder used for the reward function",
    )

    # NLE TorchBeast Encoder args
    parser.add_argument(
        "--encoder_embedding_dim", default=128, type=int, help="embedding dim"
    )
    parser.add_argument(
        "--encoder_hidden_dim", default=512, type=int, help="hidden dim for encoder"
    )
    parser.add_argument(
        "--encoder_final_activ",
        default="ln",
        type=str,
        help="final activation function for encoder",
    )
    parser.add_argument("--encoder_crop_dim", default=16, type=int, help="crop size")
    parser.add_argument(
        "--encoder_num_layers", default=2, type=int, help="number of layers"
    )
    parser.add_argument(
        "--encoder_msg_model", default="lt_cnn", type=str, help="message model"
    )
    parser.add_argument(
        "--use_crop",
        default=False,
        type=str2bool,
        help="To use a cropped view of the glyphs or not",
    )
    parser.add_argument(
        "--use_glyphs",
        default=False,
        type=str2bool,
        help="To use the full glyphs map or not",
    )
    parser.add_argument(
        "--use_blstats", default=False, type=str2bool, help="To use the bl stats or not"
    )
