...
from sample_factory.algorithms.appo.population_based_training import (
...
class APPO(ReinforcementLearningAlgorithm):
    ...
    def add_cli_args(cls, parser):
        ...
        p.add_argument(
            ...
            default=1,
            type=int,
            help="When batching on the learner is the bottleneck, increasing the number of cores PyTorch uses can improve the performance",
        )
        p.add_argument(
            "--actor_worker_gpus",
            default=[],
        ...
        p.add_argument(
            ...
            default=6,
            type=int,
            help="Number of timesteps to sample from each batch. This should be less than recurrence to decorrelate experience.",
        )
        p.add_argument(
            "--cpc_forward_subsample",
            default=2,
            type=int,
            help="Number of forward steps to sample for loss computation. This should be less than cpc_forward_steps to decorrelate gradients.",
        )
    ...
    def __init__(self, cfg):
        ...
        self.experiment_summaries_interval = (
            self.cfg.experiment_summaries_interval
        )  # sec
    ...
    def init_subset(self, indices, actor_queues):
        ...
        reset_timelimit_seconds = (
            self.cfg.reset_timeout_seconds
        )  # fail worker if not a single env was reset in that time
            ...
            try:
                while not self._should_end_training():
                    if self.cfg.llm_reward_type in [
                        ...
                        llm_proc = self.learner_workers[0].llm_worker.process
                        if llm_proc is not None and not llm_proc.is_alive():
                            raise RuntimeError("LLM process died")
                    try:
