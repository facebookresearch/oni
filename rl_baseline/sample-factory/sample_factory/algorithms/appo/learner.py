...
from rlaif.reward_model import (
    create_reward_model,
    ClassificationRewardModel,
    FastTextBoWEmbedding,
    RewardModelDataset,
    RunningMeanStd,
)
from rlaif.dataset import (
    OnlinePairsDataset,
    flatten_pair_collate_fn,
    pair_collate_fn,
)
from rlaif.prompts import get_ranking_goal_strings
...
from utils.decode import decode_message, encode_message_batch
...
class LearnerWorker:
    def __init__(
        ...
        self.stop_experience_collection_num_messages = (
            self.resume_experience_collection_num_messages
        ) = 0
        ...
        if self.cfg.llm_reward_type in ["online_cls", "online_cls_reward_model"]:

            # TODO: put this back on top once we have merged and removed assert False
            from sample_factory.algorithms.appo.llm_annotator import LLMWorker

            log.info("online: starting LLM subprocess")
            log.info(f"model: {cfg.llm_model}")
            # TODO: arguments
            self.llm_worker = LLMWorker(
                model=cfg.llm_model,
                llm_batch_size=cfg.llm_batch_size,
                reward_type=cfg.llm_reward_type,
                prompt_version=cfg.prompt_version,
                goal_key=cfg.goal_key,
            )
            self.llm_worker.start_process(server_addr=cfg.llm_server_addr)
            self.llm_message_rewards = {}
            self.message_word_counts = defaultdict(lambda: 1)

            if self.cfg.llm_reward_type == "online_cls_reward_model":
                self.reward_model_dataset = RewardModelDataset(self.llm_message_rewards)
                self.reward_model_train_acc = 0.0
                self.reward_model_num_positive_rewards = 0

            # We're pickling the reward_model_dataset in the trainer thread.
            # Pickling is not thread-safe. It happened that the run failed
            # as we're adding data into this dictionary while pickling.
            self.llm_data_lock = threading.Lock()

            self.rew_eps_threshold = -1000

        elif self.cfg.llm_reward_type in ["online_motif"]:
            ...
            # TODO: parameterize
            self.reward_model_dataset = OnlinePairsDataset(
                remove_duplicated_obs=cfg.remove_duplicated_obs == 1, device=self.device
            )
            self.log_dict["reward_model_train_acc"] = 0.0
            self.reward_model_train_acc = 0.0  # TODO: clean up

            self.llm_data_lock = threading.Lock()
            self.rew_eps_threshold = -1000

        elif self.cfg.llm_reward_type == "offline_cls":
            log.info("loading offline LLM message rewards dict")
            fname = cfg.llm_message_rewards_file
            assert os.path.exists(fname), f"File {fname} does not exist"
            with open(fname, "r") as f:
                self.llm_message_rewards = json.load(f)
                print(
                    f"loaded {len(self.llm_message_rewards)} message rewards from {fname}:"
                )
                print(str(list(self.llm_message_rewards.items())[:10]) + "...")

            self.rew_eps_threshold = -1000
        elif self.cfg.llm_reward_type == "motif":
            if self.cfg.eps_threshold_quantile != "0.0":
                try:
                    reward_csv_file = (
                        f"{cfg.reward_dir}/reward_metrics/train_norm_quantiles.csv"
                    )
                    ...
                    raise FileNotFoundError(
                        f"Reward quantiles file not found at {reward_csv_file}"
                    )

                quantile_index = np.where(
                    np.array(reward_quantile_info[0]).astype(float)
                    == self.cfg.eps_threshold_quantile
                )[0][0]
        ...
        elif self.cfg.llm_reward_type == "cosine-bow":
            self.reward_model = FastTextBoWEmbedding(
                get_ranking_goal_strings(self.cfg.goal_key),
                self.cfg.eps_threshold_quantile,
            )

        elif self.cfg.llm_reward_type == "none":
            self.rew_eps_threshold = -1000
        else:
            raise NotImplementedError
    ...
    def _maybe_save(self):
        ...
        if self.env_steps > self.cfg.save_every_steps * self.cycle_count:
            ...
            if self.cfg.llm_reward_type == "online_cls_reward_model":
                with self.llm_data_lock:
                    with self.reward_model_lock:
                        with self.policy_lock:
                            self._save()
            elif self.cfg.llm_reward_type == "online_cls":
                with self.llm_data_lock:
                    with self.policy_lock:
                        self._save()
            else:
    ...
    def _get_checkpoint_dict(self):
        ...
        if self.cfg.llm_reward_type == "online_cls_reward_model":
            # TODO: remove deepcopy later
            checkpoint["llm_message_rewards"] = copy.deepcopy(self.llm_message_rewards)
            checkpoint["reward_model"] = self.reward_model.state_dict()
            checkpoint["reward_model_dataset"] = self.reward_model_dataset
            checkpoint["reward_model_optimizer"] = (
                self.reward_model_optimizer.state_dict()
            )
        elif self.cfg.llm_reward_type in ["online_motif", "online+offline_motif"]:
            checkpoint["reward_model"] = self.reward_model.state_dict()
            # TODO: load this properly upon preemptions
            checkpoint["reward_model_optimizer"] = (
                self.reward_model_optimizer.state_dict()
            )
        elif self.cfg.llm_reward_type == "online_cls":
            checkpoint["llm_message_rewards"] = self.llm_message_rewards
    ...
    def _train(self, gpu_buffer, batch_size, experience_size, timing):
        ...
        for epoch in range(self.cfg.ppo_epochs):
            ...
            for batch_num in range(len(minibatches)):
                ...
                if self.cfg.llm_reward_type in [
                    "motif",
                    "online_motif",
                    "online+offline_motif",
                ]:
                    # Don't normalize 'obs' since normalization happens in-place when calling self.actor_critic.forward_head
                    r_head_outputs = self.reward_model.forward_head(
                        mb.obs, normalize=False
                    )
                ...
                if self.cfg.llm_reward_type in [
                    ...
                    r_core_outputs, _ = self.reward_model.forward_core(
                        r_head_outputs, torch.zeros_like(rnn_states)
                    )
                ...
                with torch.no_grad():  # these computations are not the part of the computation graph
                    ...
                    if self.cfg.with_vtrace:
                        ...
                        message_count = mb.obs["msg_count"].squeeze().cpu()
                        message_count_coeff = 1 / (
                            message_count ** (self.cfg.beta_count_exponent)
                        )

                        # extract rewards and normalize them
                        if self.cfg.llm_reward_type in ["motif", "online_motif"]:
                            llm_reward = self.reward_model.reward_fn(r_core_outputs)
                            llm_rewards_cpu = llm_reward.detach().cpu().squeeze()

                            if self.cfg.rew_norm:
                                if self.cfg.llm_reward_type in ["motif"]:
                                    llm_rewards_cpu = (
                                        llm_rewards_cpu - self.reward_model.mean
                                    ) / (self.reward_model.var) ** (1 / 2)

                                    messages = mb.obs["message"].int().cpu().numpy()
                                    messages = [
                                        decode_message(message) for message in messages
                                    ]

                                    llm_rewards_cpu = (
                                        llm_rewards_cpu > self.rew_eps_threshold
                                    ) * llm_rewards_cpu

                                elif self.cfg.llm_reward_type in ["online_motif"]:

                                    llm_reward = llm_reward.squeeze().detach()
                                    self.log_dict["intrinsic_reward_raw"] = (
                                        llm_reward.mean().item()
                                    )

                                    self.reward_rms.update(llm_reward.reshape(-1, 1))
                                    self.log_dict["reward_rms_mean"] = (
                                        self.reward_rms.mean.item()
                                    )
                                    self.log_dict["reward_rms_var"] = (
                                        self.reward_rms.var.item()
                                    )
                                    llm_reward = (llm_reward - self.reward_rms.mean) / (
                                        self.reward_rms.var
                                    ) ** (1 / 2)

                                    self.log_dict["intrinsic_reward_norm"] = (
                                        llm_reward.mean().item()
                                    )
                                    llm_reward = (
                                        llm_reward > self.cfg.eps_threshold_quantile
                                    ) * llm_reward
                                    self.log_dict["intrinsic_reward_thresh"] = (
                                        llm_reward.mean().item()
                                    )
                                    self.log_dict["intrinsic_reward_sparsity"] = (
                                        (llm_reward > 0).float().mean().item()
                                    )
                                    llm_reward_cpu = llm_reward.cpu()

                            # (misplaced) TODO: clean up reward model creation, remove reward model checkpoint

                            # if we are doing online Motif, additionally need to deal with new messages
                            # TODO: rm online+offline
                            if self.cfg.llm_reward_type in ["online_motif"]:

                                self.reward_model_dataset.add_obs_batch(
                                    mb.obs, subsample=self.cfg.online_dataset_subsample
                                )

                                # print(f'inside _train , self.device is: {self.device}')
                                llm_queue_size = self.llm_worker.message_queue.qsize()
                                if llm_queue_size < 2 * self.cfg.llm_batch_size:
                                    unseen_pairs = self.reward_model_dataset.sample_pairs_for_labeling(
                                        num_pairs=10
                                    )
                                    self.llm_worker.message_queue.put_many(unseen_pairs)
                                    log.info(
                                        f"LLM queue has {llm_queue_size} pairs, adding {len(unseen_pairs)} more."
                                    )
                            ...
                            rewards = [
                                self.llm_message_rewards.get(message, default_reward)
                                for message in messages
                            ]
                            llm_rewards_cpu = torch.tensor(rewards).float()
                            ...
                            unseen_messages = list(
                                set(
                                    [
                                        message
                                        for message in messages
                                        if message not in self.llm_message_rewards
                                    ]
                                )
                            )
                            log.info(
                                f"Got {len(unseen_messages)} unseen messages for LLM to annotate. "
                            )
                            try:
                                self.llm_worker.message_queue.put_many(unseen_messages)
                            except Full:
                                log.info("LLM worker's message queue is full!")
                            message_count = mb.obs["msg_count"].squeeze().cpu()
                            message_count_coeff = 1 / (
                                message_count ** (self.cfg.beta_count_exponent)
                            )
                        elif self.cfg.llm_reward_type == "online_cls_reward_model":
                            default_reward = 0.0
                            # TODO: raw/decoding could be optimized
                            messages_raw = mb.obs["message"].int()
                            messages_raw_np = messages_raw.cpu().numpy()
                            messages = [
                                decode_message(message) for message in messages_raw_np
                            ]

                            # prevent the initial(feedback processing) tread from
                            # updating the reward model while we eval it
                            with self.reward_model_lock:
                                self.reward_model.eval()
                                rewards = self.reward_model.make_prediction(
                                    messages_raw
                                )
                            llm_rewards_cpu = rewards.detach().cpu()
                            ...
                            self.reward_model_num_positive_rewards += torch.sum(
                                llm_rewards_cpu > 0
                            )
                            ...
                            for msg in messages:
                                llm_rewards.append(
                                    self.reward_model.compute_reward(msg)
                                )
                            llm_rewards_cpu = torch.tensor(llm_rewards)

                        elif self.cfg.llm_reward_type == "none":
                            llm_rewards_cpu = torch.zeros_like(mb.rewards_cpu)
                            message_count_coeff = 0.0
                        else:
                            raise NotImplementedError

                        rewards_cpu = (
                            self.cfg.extrinsic_reward_coeff * mb.rewards_cpu
                            + self.cfg.llm_reward_coeff
                            * llm_rewards_cpu
                            * message_count_coeff
                        )
    ...
    def load_checkpoint(checkpoints, device, checkpoint_num=0):
        if len(checkpoints) <= 0:
            ...
            if checkpoint_num == 0:
                checkpoints = sorted(
                    checkpoints,
                    key=lambda x: (int(re.search(r"_(\d+)\.pth", x).group(1)), x),
                )
                latest_checkpoint = checkpoints[-1]
            else:
                file_id = f"_{checkpoint_num}.pth"
                filtered_list = [
                    file_name for file_name in checkpoints if file_id in file_name
                ]
    ...
    def _load_state(self, checkpoint_dict, load_progress=True):
        ...
        if self.cfg.llm_reward_type == "online_cls_reward_model":
            self.llm_message_rewards = checkpoint_dict["llm_message_rewards"]
            self.reward_model.load_state_dict(checkpoint_dict["reward_model"])
            self.reward_model_dataset = checkpoint_dict["reward_model_dataset"]
            self.reward_model_optimizer.load_state_dict(
                checkpoint_dict["reward_model_optimizer"]
            )
        elif self.cfg.llm_reward_type == "online_motif":
            self.reward_model.load_state_dict(checkpoint_dict["reward_model"])
            self.reward_model_optimizer.load_state_dict(
                checkpoint_dict["reward_model_optimizer"]
            )
    ...
    def init_model(self, timing):
        ...
        if self.cfg.llm_reward_type in ["motif"]:
            checkpoints = self.get_checkpoints(
                join(self.cfg.reward_dir, "checkpoint_p0")
            )
            assert len(checkpoints) > 0
            reward_model_ckpt = self.load_checkpoint(
                checkpoints, self.device, checkpoint_num=self.cfg.checkpoint_num
            )

            reward_action_space = reward_model_ckpt["model"][
            ...
            self.reward_model = create_reward_model(
                self.cfg,
                self.obs_space,
                gym.spaces.Discrete(reward_action_space),
                timing=timing,
            )

            self.reward_model.load_state_dict(reward_model_ckpt["model"])
            self.reward_model.add_mean_var(
                reward_model_ckpt["reward_mean"].item(),
                reward_model_ckpt["reward_var"].item(),
            )

            log.info(f"Loaded offline Motif reward model from: {self.cfg.reward_dir}")

            self.reward_model.model_to_device(self.device)
            self.reward_model.share_memory()
            self.reward_model_lock = threading.Lock()
            ...
            self.reward_model = create_reward_model(
                self.cfg,
                self.obs_space,
                gym.spaces.Discrete(1),  # dummy
                timing=timing,
            )

            self.reward_model.model_to_device(self.device)
            self.reward_model_optimizer = torch.optim.Adam(
                self.reward_model.parameters(), self.cfg.reward_model_lr
            )
            ...
            self.reward_model = ClassificationRewardModel(
                n_cls=2,
                hidden_dims=[512, 512],
                classification_threshold=self.cfg.classification_threshold,
                dropout=0.1,
                device=self.device,
            ).to(device=self.device)
            self.reward_model_optimizer = torch.optim.Adam(
                self.reward_model.parameters(),
                lr=self.cfg.reward_model_lr,
            )
            self.reward_model.share_memory()
            self.reward_model_lock = threading.Lock()
        else:
    ...
    def _process_training_data(self, data, timing, wait_stats=None):
        ...
        def _get_llm_reward_dist(llm_reward_vals, n_cls):
            if n_cls == 2:
                vals = [0, 1]
            elif n_cls == 5:
                vals = [-2, -1, 0, 1, 2]
            ...
            pmf = {}
            if len(llm_reward_vals):
                llm_reward_vals = np.array(llm_reward_vals)
                for val in vals:
                    pmf[f"llm_reward_dist_{val}"] = np.sum(
                        llm_reward_vals == val
                    ) / len(llm_reward_vals)
            else:
                for val in vals:
                    pmf[f"llm_reward_dist_{val}"] = 0.0
        ...
        with timing.add_time("train"):
            ...
            # update reward model
            if (
                self.cfg.llm_reward_type in ["online_cls_reward_model", "online_motif"]
                and len(self.reward_model_dataset) > 25000
            ):
                log.info("$$$ Update reward model in the training thread")
                self.reward_model_train_acc = self.update_reward_model(num_grad_steps=1)
            ...
            if train_stats is not None:
                ...
                if self.cfg.llm_reward_type == "online_cls":
                    stats["train"]["num_llm_labelled_msg"] = len(
                        self.llm_message_rewards
                    )

                    llm_reward_vals = list(self.llm_message_rewards.values())
                    stats["train"].update(
                        _get_llm_reward_dist(llm_reward_vals, n_cls=2)
                    )
                elif self.cfg.llm_reward_type == "online_cls_reward_model":
                    stats["train"]["num_llm_labelled_msg"] = len(
                        self.reward_model_dataset
                    )
                    stats["train"][
                        "reward_model_training_accuracy"
                    ] = self.reward_model_train_acc

                    stats["train"][
                        "reward_model_num_positive_rewards"
                    ] = self.reward_model_num_positive_rewards

                    llm_reward_vals = self.reward_model_dataset.get_labels_only()
                    stats["train"].update(
                        _get_llm_reward_dist(llm_reward_vals, n_cls=2)
                    )
                elif self.cfg.llm_reward_type == "motif":
                    ...
                    # TODO: rename number of LLM labelled pairs
                    stats["train"][
                        "reward_model_dataset_samples"
                    ] = self.reward_model_dataset.num_samples()
                    stats["train"]["num_llm_labelled_pairs"] = len(
                        self.reward_model_dataset
                    )
                    # TODO: rename log_dict and make it consistent
                    stats["train"].update(self.log_dict)
    ...
    def _run(self):
        ...
        # workers should ignore Ctrl+C because the termination is handled in the event loop by a special message
        ...
        while not self.terminate:
            ...
            if self._accumulated_too_much_experience(rollouts):
                ...
                if not self.stop_experience_collection[self.policy_id]:
                    self.stop_experience_collection_num_messages += 1
                    # TODO: add a logger function for this
                    if self.stop_experience_collection_num_messages >= 50:
                        log.info(
                        ...
                        self.stop_experience_collection_num_messages = 0
                ...
                with self.resume_experience_collection_cv:
                    self.resume_experience_collection_num_messages += 1
                    if self.resume_experience_collection_num_messages >= 50:
                        log.debug(
                            "Learner %d is resuming experience collection!",
                            self.policy_id,
                        )
                        self.resume_experience_collection_num_messages = 0
                    self.resume_experience_collection_cv.notify_all()

            with torch.no_grad():
                rollouts = self._process_rollouts(rollouts, timing)

            # get new annotations from the LM
            if self.cfg.llm_reward_type == "online_cls":
                try:
                    list_new_reward_dicts = self.llm_worker.reward_queue.get_many(
                        timeout=0.005
                    )
                    num_new_rewards = sum(
                        len(reward_dict) for reward_dict in list_new_reward_dicts
                    )
                    log.info(f"Got {num_new_rewards} new rewards")
                    with self.llm_data_lock:
                        for new_reward_dict in list_new_reward_dicts:
                            log.info(new_reward_dict)
                            self.llm_message_rewards.update(new_reward_dict)
                    log.info(
                        f"Total labeled messages now: {len(self.llm_message_rewards)}"
                    )
                except Empty:
                    continue
            elif self.cfg.llm_reward_type in [
                ...
                try:
                    ...
                    with self.llm_data_lock:
                        for new_reward_dict in list_new_reward_dicts:
                            log.info(new_reward_dict)
                            self.reward_model_dataset.update(
                                new_reward_dict, self.llm_message_rewards
                            )
                            self.llm_message_rewards.update(new_reward_dict)
                    log.info(
                        f"Total labeled messages now: {len(self.llm_message_rewards)}"
                    ...
                    # update reward model
                    if len(self.reward_model_dataset) < 25000:
                        log.info(
                            "$$$ Update reward model after getting new labelled messages"
                        )
                        # prevent this tread from updating the reward model while we eval it
                        # in the training thread (to predict intrinsic rewards)
                        with self.reward_model_lock:
                            self.reward_model_train_acc = self.update_reward_model(
                                num_grad_steps=32
                            )
                except Empty:
                    continue
    ...
    def _update_reward_model_motif(self, num_grad_steps):
        self.reward_model.train()

        # print(f'inside _update_reward_model_motif, self.device is: {self.device}')
        ...
        loss_fn = (
            lambda logprobs, target: -(target * logprobs).sum() / logprobs.shape[0]
        )
        ...
        for _ in range(num_grad_steps):
            ...
            result = self.reward_model.forward_pairs(mb, normalize=False)
            rewards = result.rewards
            rewards = rewards.mean(axis=0)

            labels = mb["label"].to(self.device).type(torch.float)  # BS
            ...
            predicted_log_probs = torch.nn.functional.log_softmax(rewards, dim=1)
            ...
            grad_norm += torch.nn.utils.clip_grad_norm_(
                self.reward_model.parameters(), 10.0
            ).item()
        ...
        train_loss /= num_grad_steps
        train_acc /= num_grad_steps
        grad_norm /= num_grad_steps

        log.info(f"$$$ Done updating the reward model, accuracy: {train_acc}")

        self.log_dict["reward_model_train_acc"] = train_acc
        self.log_dict["reward_model_train_loss"] = train_loss
        self.log_dict["reward_model_grad_norm"] = grad_norm

        return train_acc

    def update_reward_model(self, num_grad_steps):

        # TODO: something a bit cleaner
        if self.cfg.llm_reward_type == "online_motif":
            return self._update_reward_model_motif(num_grad_steps)
        ...
        dataloader = torch.utils.data.DataLoader(
            ...
            batch_size=self.cfg.reward_model_batch_size,
            # num_workers=24,
            # comment this line out bc I got AssertionError: daemonic processes are not allowed to have children for the enuemrate dataloader line
            shuffle=True,
        )
        ...
        n_sample = 0
        while cnt < N:
            for _, (messages, rewards) in enumerate(dataloader):
                rewards = torch.tensor(rewards).to(device=self.device)
                messages_raw = torch.tensor(
                    encode_message_batch(messages), device=self.device
                )

                logits = self.reward_model(messages_raw)
                loss = loss_fn(logits, rewards)
                ...
                with torch.no_grad():
                    ...
                    correct += (prediction == rewards).sum().item()
        ...
        log.info("$$$ Done updating the reward model")
